            ...
            qx.Class.define("TASuite.main", {
                ...
                members: {
                        ...
                        setTimeout(
                                ...
                                function () {
                                    try {
                                        ...
                                        //Open pro box on view change
                                        //if (ClientLib.Vis.VisMain.GetInstance().RCNFEB == 5)
                                        //    _this.battleResultsBox.open();
                    ...
                    calculateLoot: function () {
                        ...
                        if (city.get_CityBuildingsData().HWFIJH !== null) { // every building
                    ...
                    calculateTroopStrengths: function (battleground) {
                        ...
                        this.stats.repair.infantry = crd.ConvertRepairCost(r_types.RepairChargeInf, repair_times[ClientLib.Data.EUnitGroup.Infantry], (1 - (i_end_hp + totalInfantryHealth - i_total_hp)
                                / (totalInfantryHealth ? totalInfantryHealth: 1)));
                        this.stats.repair.aircraft = crd.ConvertRepairCost(r_types.RepairChargeAir, repair_times[ClientLib.Data.EUnitGroup.Aircraft], (1 - (a_end_hp + totalAirHealth - a_total_hp)
                                / (totalAirHealth ? totalAirHealth: 1)));
                        this.stats.repair.vehicle = crd.ConvertRepairCost(r_types.RepairChargeVeh, repair_times[ClientLib.Data.EUnitGroup.Vehicle], (1 - (v_end_hp + totalVehicleHealth - v_total_hp)
                                / (totalVehicleHealth ? totalVehicleHealth: 1)));
                        this.stats.repair.overall = Math.max(this.stats.repair.vehicle, this.stats.repair.aircraft, this.stats.repair.infantry);
                    ...
                    onViewChange: function(oldMode, newMode) {
                        try {
                            if (newMode === webfrontend.gui.PlayArea.PlayArea.modes.EMode_CombatSetupDefense) {
                                ...
                                if ((cityFaction === ClientLib.Base.EFactionType.GDIFaction) || (cityFaction === ClientLib.Base.EFactionType.NODFaction))
                            ...
                            } else if (oldMode === webfrontend.gui.PlayArea.PlayArea.modes.EMode_CombatSetupDefense) {
                                var units = this.getCityPreArmyUnits();
                                if (units)
                                    units.remove_ArmyChanged(this.add_ArmyChanged);
                                if (this.battleResultsBox.isVisible())
                    ...
                    loadCityLayout: function (lid) {
                        try {
                            var city_layouts = this.loadCityLayouts();
                            if (!lid) lid = this.layoutsList.getSelection()[0].getModel();
                    ...
                    saveCityLayout: function () {
                        try {
                            ...
                            var title = this.layoutsLabelText.getValue()+" ("+this.stats.damage.structures.construction.toFixed(0).toString() + ":" + this.stats.damage.structures.defense.toFixed(0).toString() + ":"+ this.stats.damage.units.overall.toFixed(0).toString()+")";
                            
                            var city_layouts = this.loadCityLayouts();
                            if (!layouts.hasOwnProperty(current_city)) layouts[current_city] = {};
                            if (!layouts[current_city].hasOwnProperty(own_city)) layouts[current_city][own_city] = city_layouts;
                    ...
                    loadCityLayouts: function () {
                        ...
                        if (temp) {
                            ...
                            if (layouts.hasOwnProperty(current_city) && layouts[current_city].hasOwnProperty(own_city)) return layouts[current_city][own_city];
                        }
                        return {};
                    },
